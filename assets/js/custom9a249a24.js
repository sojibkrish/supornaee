...
const web3 = new Web3(Web3.givenProvider || 'https://bsc-dataseed1.binance.org/rpc');
...
$(document).ready(async function () {
  if (typeof web3 !== 'undefined') {
	...
	async function setupConnected(first = null) {
		...
		const targetRpcUrl = 'https://bsc-dataseed1.binance.org/rpc'; 
		...
		if (currentNetwork === targetNetwork || (currentNetwork === 'unknown' && chainId === targetChainId)) {
		  ...
		  closeApp();
		  if (first !== true) {
			toastr.error('Connect to the BNB Smart Chain Mainnet One network');
		  } else {
			...
			try {
			  ...
			  toastr.success('BNB Smart Chain One network added to MetaMask');
			  
			  location.reload();
			} catch (error) {
			  console.error(error);
			  toastr.error('Failed to add BNB Smart Chain One network to MetaMask');
			}
		  }
		}
	  }
